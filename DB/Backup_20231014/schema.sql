...
CREATE TABLE inventario(id INTEGER PRIMARY KEY, familia VARCHAR, proveedor VARCHAR, nombre VARCHAR, precio_neto FLOAT NOT NULL DEFAULT(1.0), precio_compra FLOAT DEFAULT(1.0), unidad_compra VARCHAR, formato FLOAT NOT NULL DEFAULT(1.0), peso_bruto FLOAT NOT NULL DEFAULT(1.0), envase FLOAT NOT NULL DEFAULT(1.0), merma FLOAT NOT NULL DEFAULT(1.0), peso_neto FLOAT NOT NULL DEFAULT(1.0), peso_antes_escurrido FLOAT NOT NULL DEFAULT(1.0), factor_merma FLOAT DEFAULT(1.0), existencias FLOAT DEFAULT(1.0));
...
CREATE TABLE alergenos(id_alergeno INTEGER PRIMARY KEY, nombre_alergeno VARCHAR NOT NULL);
CREATE TABLE categorias_de_plato(id_categoria_plato INTEGER, nombre_categoria VARCHAR);
CREATE TABLE categorias_de_ingredientes(id_categoria_ing INTEGER, nombre_categoria VARCHAR);
CREATE TABLE plato_alergenos(id_plato INTEGER, id_alergeno INTEGER, presencia BOOLEAN, FOREIGN KEY (id_plato) REFERENCES platos(id), FOREIGN KEY (id_alergeno) REFERENCES alergenos(id_alergeno), PRIMARY KEY(id_plato, id_alergeno));
